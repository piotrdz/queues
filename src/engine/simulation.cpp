...
static SimulationInstance readFromFile(std::string path)
{
   ...
   if(confFile.is_open())
   {
	 ...
	 if (lineNo == 0)
	 {
	     ...
	     spacePos = line.find(" ");
	     stationsNo = atoi(line.substr(spacePos+1, line.length()-spacePos).c_str());
	 }
...
static void saveToFile(std::string path, SimulationInstance& simInstance)
{
    ...
    if (confFile.is_open())
    {
	 ...
	 {
	      line.clear();
	      line[0] = TypeToString(simInstance.stations[i].serviceTimeDistribution.type);
	      strStream1.clear();
	      ...
	      line.append("," + simInstance.stations[i].processorCount);
         ...
         for (int i=0; i < simInstance.connections.size(); i++)
	 {
	      ...
	      if (simInstance.connections[i].from == 0)
	      {
		  line.append("WE,");
	      }
	      else 
